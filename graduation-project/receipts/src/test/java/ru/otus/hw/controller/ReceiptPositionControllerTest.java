...
class ReceiptPositionControllerTest {
    ...
    @MockBean
    private ReceiptPositionServiceImpl service;
    ...
    void create() throws Exception {
        ...
        val createDto = new ReceiptPositionCreateDto(dto.getReceiptDto().getId(),
    ...
    void update() throws Exception {
        ...
        val updateDto = new ReceiptPositionUpdateDto(dto.getId(),
    ...
    void updateExceptionWithIdNull() throws Exception {
        val dto = getDto();
        val updateDto = new ReceiptPositionUpdateDto(null,
                dto.getReceiptDto().getId(), dto.getServingDto().getId(), dto.getQuantity());
        mvc.perform(patch("/api/ingredients/%d".formatted(dto.getId()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(updateDto)))
    ...
    private List<ReceiptPositionDto> getExampleList() {
        return LongStream.range(1L, 4L).boxed()
                .map(this::getDto)
                .toList();
    }

    private ReceiptPositionDto getDto() {
        return getDto(FIRST_ID);
    }

    private ReceiptPositionDto getDto(Long id) {
        return new ReceiptPositionDto(id, getReceiptDto(id),
