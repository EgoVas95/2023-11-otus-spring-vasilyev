...
class FoodControllerTest {
    ...
    void findById() throws Exception {
        ...
        mvc.perform(get("/api/foods/%s".formatted(FIRST_ID)))
                .andExpect(status().isOk())
    ...
    void update() throws Exception {
        ...
        mvc.perform(patch("/api/foods/%s".formatted(expect.getId()))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteObj() throws Exception {
        mvc.perform(delete("/api/foods/%s".formatted(FIRST_ID)))
                .andExpect(status().isNoContent());
    }

    private List<Food> getDbEx() {
        return LongStream.range(1L, 4L).boxed()
                .map(this::getExampleObj)
                .toList();
    }

    private Food getExampleObj() {
        return getExampleObj(1L);
    }

    private Food getExampleObj(Long id) {
        return new Food(id.toString(), id.toString());
    }
}
