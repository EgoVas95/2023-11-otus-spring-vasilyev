...
class ServingControllerTest {
    ...
    void findById() throws Exception {
        ...
        mvc.perform(get("/api/servings/%s".formatted(FIRST_ID)))
                .andExpect(status().isOk())
    ...
    void update() throws Exception {
        ...
        mvc.perform(patch("/api/servings/%s".formatted(expect.getId()))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteObj() throws Exception {
        mvc.perform(delete("/api/servings/%s".formatted(FIRST_ID)))
                .andExpect(status().isNoContent());
    }

    private List<Serving> getDbEx() {
        return LongStream.range(1L, 4L).boxed()
                .map(this::getExampleObj)
                .toList();
    }

    private Serving getExampleObj() {
        return getExampleObj(1L);
    }

    private Serving getExampleObj(Long id) {
        return new Serving(id.toString(), id.toString(),
                new Food(id.toString(), id.toString()), id);
    }
}
